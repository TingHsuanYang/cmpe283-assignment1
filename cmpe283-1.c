...
// Table 24-6
struct capability_info procbased_ctls[21] =
    {
...
// Table 24-8
struct capability_info procbased_ctls3[9] =
    {
        {0, "LOADIWKEY exiting"},
        {1, "Enable HLAT"},
...
void detect_vmx_features(void) {
    ...
    pr_info("Exit Controls MSR: 0x%llx\n",
            (uint64_t)(lo | (uint64_t)hi << 32));
    report_capability(exit_ctls, 11, lo, hi);

    /* Entry Controls */
    rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
    pr_info("Entry Controls MSR: 0x%llx\n",
            (uint64_t)(lo | (uint64_t)hi << 32));
    report_capability(entry_ctls, 9, lo, hi);
}
